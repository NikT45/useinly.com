...
export default function HeaderNav() {
    ...
    return (
        <nav className="min-w-[25%] p-[6px] rounded-full flex items-center justify-between">
            <button onClick={() => router.push('/home')} className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 group ${isHome ? 'bg-brand-coral' : ''}`}>
                <MessageCircle size={24} className={`${isHome ? 'text-white' : 'text-brand-coral group-hover:text-brand-wine'}`} strokeWidth={2} />
            </button>
            <button onClick={() => router.push('/reflections')} className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 group ${isReflections ? 'bg-brand-coral' : ''}`}>
                <NotebookPen size={24} className={`${isReflections ? 'text-white' : 'text-brand-coral group-hover:text-brand-wine'}`} strokeWidth={2} />
            </button>
